...
// POST /api/jobs/email-sync/gmail
export async function POST(request: NextRequest) {
  try {
    ...
    const tenantId =
      typeof body.tenantId === 'string' && ObjectId.isValid(body.tenantId)
    ...
    const result = await withTimeout(syncGmailInboxForIntegration(integrationId, {}, tenantId), JOB_TIMEOUT_MS);
    return createSuccessResponse(result);
  } catch (error) {
    return handleApiError(error, 'Gmail background sync job failed');
  }
}
