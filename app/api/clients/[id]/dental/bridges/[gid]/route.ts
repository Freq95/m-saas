...
import { updateBridgeGroupSchema } from '@/lib/dental/schemas';
import { updateBridgeGroup, deleteBridgeGroup } from '@/lib/server/bridges';
...
// PATCH /api/clients/[id]/dental/bridges/[gid] — update tooth list and/or comment.
export async function PATCH(request: NextRequest, props: RouteParams) {
  const params = await props.params;
  try {
    const auth = await getAuthUser();
    if (!isClinicalRole(auth.role)) {
      return createErrorResponse('Doar medicii pot modifica punți.', 403);
    }
    ...
    const parsed = updateBridgeGroupSchema.safeParse(body);
    if (!parsed.success) {
      return createErrorResponse('Invalid input', 400, parsed.error.issues);
    }

    const updated = await updateBridgeGroup(
      { tenantId: scope.tenantId, userId: scope.userId, clientId: ids.clientId },
      ids.groupId,
      parsed.data
    );
    if (!updated) return createErrorResponse('Bridge group not found', 404);

    const data = await getDentalData(ids.clientId, scope.tenantId, scope.userId);
    return createSuccessResponse({ bridge_group: updated, dental: data });
  } catch (error) {
    return handleApiError(error, 'Failed to update bridge group');
  }
}

// DELETE /api/clients/[id]/dental/bridges/[gid] — remove a bridge annotation.
export async function DELETE(_request: NextRequest, props: RouteParams) {
  const params = await props.params;
  try {
    const auth = await getAuthUser();
    if (!isClinicalRole(auth.role)) {
      return createErrorResponse('Doar medicii pot șterge punți.', 403);
    }
    const limited = await checkWriteRateLimit(auth.userId);
    if (limited) return limited;
    ...
    const deleted = await deleteBridgeGroup(
      { tenantId: scope.tenantId, userId: scope.userId, clientId: ids.clientId },
      ids.groupId
    );
    if (!deleted) return createErrorResponse('Bridge group not found', 404);

    const data = await getDentalData(ids.clientId, scope.tenantId, scope.userId);
    return createSuccessResponse({ dental: data });
  } catch (error) {
    return handleApiError(error, 'Failed to delete bridge group');
  }
}
